// FIXME: Unsat; FnMut closure pre!/post! specs are Unsat branch-wide (closure_postcondition_fnmut.rs fails too). Without the redundant method-level `where F: FnMut` bound Thrust panics at src/analyze/annot_fn.rs:571 ("precondition used on a non-closure parameter").
...
impl<F: FnMut(i64) -> i64> S<F> {
    ...
    // The impl-level `F: FnMut` bound is not seen by `build_closure_type_for_param`
    // (src/refine/template.rs:598, own `predicates_of` only); repeat it on the method.
