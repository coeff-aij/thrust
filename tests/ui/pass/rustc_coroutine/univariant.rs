...
// (README.md, "段階 6"). The value types (Size/Align/Integer/../Niche/
// TargetDataLayout) are copied from the already-annotated tests/ui/pass/
// rustc_coroutine/values.rs (same underlying code, stage 1) rather than
// re-derived, for the same reason as eligibility.rs reusing bitset.rs.
// `IndexSlice`/`IndexVec`/`Idx` are copied from eligibility.rs (stage 2/4/5)
// including its `IndexSlice` `Model` workaround (see the comment on that impl
// there for why `type Ty = Self` is impossible). This file is a DRAFT: it is
// not expected to verify (`ignore-on-host` above); `univariant`,
// `univariant_biased` and `scalar_pair` all stay `#[thrust::trusted]` per the
...
// //== ./../rustc_index/src/idx.rs (from bitset.rs, stage 2/4)
...
pub trait Idx: Copy + 'static + Eq + PartialEq + Debug + Hash {
    ...
    /// Precondition of `Self::new(idx)`. See eligibility.rs's copy of this
    /// trait for why only the `usize` instantiation is covered here.
    #[thrust_macros::predicate]
...
// //== ./../rustc_index/src/slice.rs (from eligibility.rs, stage 2)
...
// //== ./src/layout.rs / ./src/layout/simple.rs value types (verbatim from
// values.rs, stage 1, with its existing `dl_wf` / `prim_wf` specs)
...
// formula body, which has no `.length`/`.array` fields; see values.rs's
...
impl Size {
    ...
    // Trusted: the body's `checked_mul` panics on overflow; overflow is out
    // of scope (see the stage plan), so the spec states the no-overflow
    // result.
    #[inline]
    #[thrust::trusted]
...
impl Integer {
    ...
    // Trusted: the match arms use u128 constants beyond the i64 range Thrust
    // models integer literals with.
    #[inline]
    #[thrust::trusted]
    #[thrust::callable]
...
// //== ./src/lib.rs (verbatim from the target)
...
// //== ./src/layout/simple.rs (verbatim, stays trusted per the stage plan)
...
// `univariant_biased` stay trusted, per the stage plan)
...
// Stage 6 spec (README.md, "段階 6"): the permutation part of `univariant`'s
...
// TODO(spec): the intended `requires`/`ensures` (README, 段階 6) is:
//
            ...
            if !matches!(kind, StructKind::MaybeUnsized) {
                if let Some(niche) = layout.largest_niche {
                    ...
                    if fields.len() > 1 && head_space != 0 && tail_space > 0 {
                        ...
                        let alt_niche = alt_layout.largest_niche.unwrap_without_debug();
...
// See eligibility.rs's copy of this impl for why `type Ty = Self` is
// impossible (`IndexSlice`'s `raw: [T]` makes it unsized) and why the `[T]`
