...
// A variant of `annot_range_loop.rs` keeping the two-directional form of the
// `next` contract: both `result == None <==> Self::completed` and both
// directions of `Self::step`. The decided shape is implications only, and the
// disjunctive original in `annot_range_loop.rs` is what verifies -- this form
// times out at 60s. Kept to pin which direction of the contract costs the
// solver the loop.
