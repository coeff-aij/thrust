...
// Unsat: `Range{start,end}` produces exactly `end - start` items, one fewer than claimed.
#[thrust_macros::ensures(result.len() == end - start + 1 && forall(|k: Int| 0 <= k && k < result.len() ==> result[k] == start + k))]
