    ...
    F: Fn(i64) -> i64
{
    ...
    #[thrust::predicate]
    fn step(self, item: Self::Item, dist: Self) -> bool {
        // insert predicate variable read from .smt2
        // or replace predicate's name using regex
        "(exists ((v Int) (v' Int) (f F))
            (and
                (= (tuple_proj<Int-Int>.1 self) (tuple_proj<Int-Int>.1 dist))
                (= (tuple_proj<Int-Int>.0 self) v)
                (pre_f(f, v) and post_f(Mut<F, F>(f, f'), v, v'))
                (= v' item)
...
fn main() {
    ...
    // DefId of closure -> pre/post (stored as refinement types)
