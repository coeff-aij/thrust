...
- `THRUST_OUTPUT_DIR`: Directory where Thrust writes its debug output (the MIR dump and, when `THRUST_DUMP_SMT2` is set, the SMT2 dump). When unset, output goes to the current directory.
- `THRUST_DUMP_MIR`: When set, Thrust writes the optimized MIR of every analyzed function into a single `<crate>.mir` file, mirroring rustc's `--emit mir` output. The value is a substring filter on the def path; use `all` (or `1`/`true`) to include every analyzed def. The file is written to `THRUST_OUTPUT_DIR` when set, else to the current directory.
- `THRUST_DUMP_SMT2`: When set, Thrust writes the generated SMT-LIB2 constraints into `thrust_output.smt2`, in `THRUST_OUTPUT_DIR` when set, else in the current directory.
